...
const search = async(x: FormEvent<HTMLFormElement>) => {
    x.preventDefault()
    ...
    try {
      const res = await fetch(`https://api.weatherapi.com/v1/current.json?key=${process.env.NEXT_PUBLIC_WEATHER_API_KEY}&q=${trimmed}`)
      if(!res.ok) {
